...
exports.addUserInfo = async (req, res) => {
    try {
        const file = req.file;
        const filename = file ? "/assets/profileCoverStorage/" + file.filename : undefined;

        const existingInfo = await userInformationModel.findOne({ user: req.body.user_id });

        if (existingInfo) {
            await userInfoModal.updateOne({ user: req.body.user_id }, {
                $set: {
...
exports.getCurrentUser = async (req, res) => {
    try {
        ...
        const response1 = await userModel.findOne({ userToken: response.userToken })
            ...
            .populate({
                path: "user_post",
                populate: [
                    { path: "getLikes", match: { likeStatus: "like" }, populate: [{ path: "user_id" }, { path: "userClickId" }] },
...
exports.findAllUsers = async (req, res) => {
    try {
        const response = await userModel.find().populate({
            path: "user_post",
            populate: [
                { path: "getLikes", populate: [{ path: "userClickId" }, { path: "user_id" }] },
                { path: "postComments" }
            ]
        });
