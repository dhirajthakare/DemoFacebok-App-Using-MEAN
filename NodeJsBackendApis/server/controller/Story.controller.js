...
exports.getStory = async (req, res) => {
    try {
      const userStories = await storyModal
        .find({ user_id: req.params.id, createdAt: { $gt: new Date(Date.now() - 24 * 60 * 60 * 1000) } })
        .populate('user_id');
  
      const friendStories = await storyModal
        .find({ user_id: req.body, createdAt: { $gt: new Date(Date.now() - 24 * 60 * 60 * 1000) } })
        .populate('user_id');
  
      res.json(
        Array(
          { userStories: userStories },
          { userFriendStories: friendStories }
        ));
    } catch (error) {
